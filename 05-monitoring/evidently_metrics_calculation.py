...
report = Report(metrics = [
    ...
    ColumnCorrelationsMetric(column_name='passenger_count')
])
...
def calculate_metrics_postgresql(curr, i):
	...
	#column_summary_mean = result['metrics'][4]['result']['reference_characteristics']['count']
	column_correlation = result['metrics'][4]['result']['current']['pearson']['values']['y'][2]
    
	curr.execute(
		"insert into dummy_metrics(timestamp, prediction_drift, num_drifted_columns, share_missing_values, column_quantile, column_correlation) values (%s, %s, %s, %s, %s, %s)",
		(begin + datetime.timedelta(i), prediction_drift, num_drifted_columns, share_missing_values, column_quantile, column_correlation)
	)
